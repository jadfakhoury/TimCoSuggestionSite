...
public partial class Details
{
    ...
    private async Task VoteUp()
    {
        ...
        {
            if (suggestion.Author.Id != loggedInUser.Id) //Can't upvote on your own suggestion
            {
                //This line will try to upvote if user has already upvoted it will
                //return false and the upvote will be removed or else it will return true
                //and the upvote will be added to the suggestion.
    ...
    private async Task CompleteSetStatus()
    {
        switch (settingStatus)
        {
            case "completed":
                ...
                suggestion.OwnerNotes = $"You are right, this is an important topic for developers." + $"We created a resources about it here: <a href='{urlText}' " + $"target='_blank'>{urlText}</a>";
                break;
            case "watching":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
